...
with open(metadata_file_path, 'r') as metadata_file:
    ...
    # Add a check to ensure each item is a dictionary and has the correct audio file path
    for i, item in enumerate(items):
        if not isinstance(item, dict):
            raise ValueError(f"Item at index {i} is not a dictionary. Item content: {item}")
        if not os.path.exists(item['audio_file']):
            raise FileNotFoundError(f"Audio file not found for item at index {i}: {item['audio_file']}")
...
g2p_mapping_path = '/data/kikuyu/kikuyu_g2p_mapping.txt'
